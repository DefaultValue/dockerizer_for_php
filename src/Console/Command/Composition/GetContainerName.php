...
class GetContainerName extends \Symfony\Component\Console\Command\Command
{
    ...
    protected function configure(): void
    {
        $this->setDescription('<info>Install Magento packed inside the Docker container</info>')
            ...
            ->setHelp(<<<'EOF'
                Run <info>%command.name%</info> return Docker container ID for the given running service
    ...
    public function execute(InputInterface $input, OutputInterface $output): int
    {
        ...
        $dockerCompose->getServiceContainerName($service);

        // Set normal verbosity to output result
        $output->setVerbosity($output::VERBOSITY_NORMAL);
        $output->write($dockerCompose->getServiceContainerName($service));
