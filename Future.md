...
Pull templates from the repository. Implement ability do add and remove multiple
repositories with templates

`composition:template:list-all` - filter packages by name or supported package?
Delete and use meta only! `composition:template:meta` - filter packages by name
or supported package
...
4. Add command for get template meta information: name, supported packages,
...
8. Get command descriptions and short descriptions to generate documentation to
   keep this information fully consistent and synchronized with the code.
9. Create command option and argument registry OR extract all of them into the
